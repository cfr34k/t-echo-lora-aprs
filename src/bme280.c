...
static ret_code_t start_transfer_for_current_state(void)
{
	...
	switch(m_state)
	{
		case BME280_STATE_RESET:
			m_twi_tx_buf[0] = 0xE0; // reset register
			m_twi_tx_buf[1] = 0xB6; // magic word
			bytes2send = 1;
			break;

		case BME280_STATE_READ_CHIPID:
			m_twi_tx_buf[0] = 0xD0; // chip ID register
			bytes2send = 1;
			bytes2receive = 1;
			break;

		case BME280_STATE_READ_CAL1:
			m_twi_tx_buf[0] = 0x88; // first calibration data register of first block
			bytes2send = 1;
			bytes2receive = 0xA1 + 1 - 0x88;
			break;

		case BME280_STATE_READ_CAL2:
			m_twi_tx_buf[0] = 0xE1; // first calibration data register of first block
			bytes2send = 1;
	...
	if(bytes2send != 0 && bytes2receive != 0) {
		...
		xfer = (nrfx_twim_xfer_desc_t)NRFX_TWIM_XFER_DESC_TXRX(
				BME280_7BIT_ADDR, m_twi_tx_buf, bytes2send, m_twi_tx_buf, bytes2receive);
...
static ret_code_t setup_twim(void)
{
	...
	return nrfx_twim_init(&m_twim, &twim_config, cb_twim, NULL);
}
